...
public class ForkJoinTaskTest extends RecursiveTask<Integer> {
	...
	public ForkJoinTaskTest(List<String> datas) {
		this.datas = datas;
	}
	...
	protected Integer compute() {
		...
		if(datas.size() <= THRESHOLD) {
			...
			ForkJoinTaskTest leftAction = new ForkJoinTaskTest(leftData);
			ForkJoinTaskTest rightAction = new ForkJoinTaskTest(rightData);
			System.out.println("第"+n.incrementAndGet()+"次任务拆分leftData:"+leftData+"   rightData:"+rightData );
			//执行两个子任务
			leftAction.fork();
			rightAction.fork();
			
			rest = leftAction.join() + rightAction.join();
		}
		return rest;
	}
	...
	public static void main(String[] args) {
		...
		try {
	        ...
	        Future<Integer> ft = forkJoinPool.submit(new ForkJoinTaskTest(datas));
